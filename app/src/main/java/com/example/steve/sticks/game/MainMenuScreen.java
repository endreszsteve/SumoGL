    ...
    public void update(float deltaTime)
    {
        ...
        for(int i = 0; i < len; i++)
        {
            ...
            if(event.type == TouchEvent.TOUCH_UP)
            {
                ...
                if(OverlapTester.pointInRectangle(playBounds, touchPoint)) // play button
                {
                    Assets.playSound(Assets.clickSound);
                    game.setScreen(new GameScreen(game));
                    return;
                }
                if(OverlapTester.pointInRectangle(highscoresBounds, touchPoint)) // highscore button
                {
                    Assets.playSound(Assets.clickSound);
                    game.setScreen(new HighscoresScreen(game));
                    return;
                }
                if(OverlapTester.pointInRectangle(helpBounds, touchPoint)) // help screen button
                {
                    Assets.playSound(Assets.clickSound);
                    game.setScreen(new HelpScreen(game));
                    return;
                }
                if(OverlapTester.pointInRectangle(soundBounds, touchPoint)) // Sound toggle
                {
                    Assets.playSound(Assets.clickSound);
